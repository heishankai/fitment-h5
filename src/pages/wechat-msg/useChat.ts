...
interface ChatMessage {
  id: number | string
  content: string
  sender_type?: 'wechat' | 'service' | string
  senderType?: 'wechat' | 'service' | string
...
export function useChat() {
  ...
  const connectWS = async () => {
    ...
    socket.value = (window as any).io(`${getWsUrl()}/chat`, {
      auth: { token: t },
      query: { token: t },
  ...
  const init = async () => {
    loading.value = true
    try {
      const res = await getWechatUserRoom()
      if (!res?.success || !res.data) throw new Error(res?.message || '获取房间失败')

      roomId.value = res.data.id
      // 保存微信用户头像
      if (res.data.wechat_user?.avatar) {
        wechatUserAvatar.value = res.data.wechat_user.avatar
      }
      await loadMessages()
      await connectWS()
    } catch (e: any) {
